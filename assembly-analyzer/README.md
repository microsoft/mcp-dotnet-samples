...
1. Build the MCP server app as a container image.
    ...
    ```bash
    cd $REPOSITORY_ROOT/assembly-analyzer
    docker build -f Dockerfile.stdio -t mcp-assemblyanalyzer-stdio:latest .
    ```
...
1. Run the MCP server app.
    ...
    ```bash
    cd $REPOSITORY_ROOT/assembly-analyzer
    dotnet run --project ./src/McpAssemblyAnalyzer.ContainerApp
    ```
...
1. Build the MCP server app as a container image.
    ...
    ```bash
    cd $REPOSITORY_ROOT/assembly-analyzer
    docker build -f Dockerfile.http -t mcp-assemblyanalyzer-http:latest .
    ```

1. Run the MCP server app in a container

    ```bash
    docker run -d -p 8080:8080 --name mcp-assemblyanalyzer-http mcp-assemblyanalyzer-http:latest
    ```
...
1. After the deployment is complete, get the information by running the following commands:
   ...
   - Azure Container Apps FQDN:
     ...
     ```bash
     azd env get-value AZURE_RESOURCE_MCP_ASSEMBLYANALYZER_FQDN
     ```
...
1. Copy `mcp.json` to the repository root.
    ...
    cp $REPOSITORY_ROOT/assembly-analyzer/.vscode/mcp.stdio.local.json \
       $REPOSITORY_ROOT/.vscode/mcp.json
    ```

    ```powershell
    New-Item -Type Directory -Path $REPOSITORY_ROOT/.vscode -Force
    Copy-Item -Path $REPOSITORY_ROOT/assembly-analyzer/.vscode/mcp.stdio.local.json `
              -Destination $REPOSITORY_ROOT/.vscode/mcp.json -Force
    ```

1. Open Command Palette by typing `F1` or `Ctrl`+`Shift`+`P` on Windows or `Cmd`+`Shift`+`P` on Mac OS, and search `MCP: List Servers`.
1. Choose `mcp-assemblyanalyzer-stdio-local` then click `Start Server`.
1. When prompted, enter the absolute directory of the `McpAssemblyAnalyzer.ConsoleApp` project.
1. Enter prompt like:
...
1. Copy `mcp.json` to the repository root.
    ...
    ```bash
    mkdir -p $REPOSITORY_ROOT/.vscode
    cp $REPOSITORY_ROOT/assembly-analyzer/.vscode/mcp.stdio.container.json \
       $REPOSITORY_ROOT/.vscode/mcp.json
    ```

    ```powershell
    New-Item -Type Directory -Path $REPOSITORY_ROOT/.vscode -Force
    Copy-Item -Path $REPOSITORY_ROOT/assembly-analyzer/.vscode/mcp.stdio.container.json `
              -Destination $REPOSITORY_ROOT/.vscode/mcp.json -Force
    ```

1. Open Command Palette by typing `F1` or `Ctrl`+`Shift`+`P` on Windows or `Cmd`+`Shift`+`P` on Mac OS, and search `MCP: List Servers`.
1. Choose `mcp-assemblyanalyzer-stdio-container` then click `Start Server`.
1. Enter prompt like:
...
1. Copy `mcp.json` to the repository root.
    ...
    ```bash
    mkdir -p $REPOSITORY_ROOT/.vscode
    cp $REPOSITORY_ROOT/assembly-analyzer/.vscode/mcp.http.local.json \
       $REPOSITORY_ROOT/.vscode/mcp.json
    ```

    ```powershell
    New-Item -Type Directory -Path $REPOSITORY_ROOT/.vscode -Force
    Copy-Item -Path $REPOSITORY_ROOT/assembly-analyzer/.vscode/mcp.http.local.json `
              -Destination $REPOSITORY_ROOT/.vscode/mcp.json -Force
    ```

1. Open Command Palette by typing `F1` or `Ctrl`+`Shift`+`P` on Windows or `Cmd`+`Shift`+`P` on Mac OS, and search `MCP: List Servers`.
1. Choose `mcp-assemblyanalyzer-http-local` then click `Start Server`.
1. Enter prompt like:
...
1. Copy `mcp.json` to the repository root.
    ...
    ```bash
    mkdir -p $REPOSITORY_ROOT/.vscode
    cp $REPOSITORY_ROOT/assembly-analyzer/.vscode/mcp.http.container.json \
       $REPOSITORY_ROOT/.vscode/mcp.json
    ```

    ```powershell
    New-Item -Type Directory -Path $REPOSITORY_ROOT/.vscode -Force
    Copy-Item -Path $REPOSITORY_ROOT/assembly-analyzer/.vscode/mcp.http.container.json `
              -Destination $REPOSITORY_ROOT/.vscode/mcp.json -Force
    ```

1. Open Command Palette by typing `F1` or `Ctrl`+`Shift`+`P` on Windows or `Cmd`+`Shift`+`P` on Mac OS, and search `MCP: List Servers`.
1. Choose `mcp-assemblyanalyzer-http-container` then click `Start Server`.
1. Enter prompt like:
...
1. Copy `mcp.json` to the repository root.
    ...
    ```bash
    mkdir -p $REPOSITORY_ROOT/.vscode
    cp $REPOSITORY_ROOT/assembly-analyzer/.vscode/mcp.http.remote.json \
       $REPOSITORY_ROOT/.vscode/mcp.json
    ```

    ```powershell
    New-Item -Type Directory -Path $REPOSITORY_ROOT/.vscode -Force
    Copy-Item -Path $REPOSITORY_ROOT/assembly-analyzer/.vscode/mcp.http.remote.json `
              -Destination $REPOSITORY_ROOT/.vscode/mcp.json -Force
    ```

1. Open Command Palette by typing `F1` or `Ctrl`+`Shift`+`P` on Windows or `Cmd`+`Shift`+`P` on Mac OS, and search `MCP: List Servers`.
1. Choose `mcp-assemblyanalyzer-http-remote` then click `Start Server`.
1. Enter the Azure Container Apps FQDN.
1. Enter prompt like:

    ```text
    Analyze this DLL file `{{path/to/dll/file}}`
    ```
...
1. Set the arguments that pointing to the console app project and **Connect**:
    ...
    ```text
    run --project {{absolute/path/to/assembly-analyzer}}/src/McpAssemblyAnalyzer.ConsoleApp
    ```
...
1. Set the arguments that pointing to the console app project and **Connect**:
    ...
    ```text
    run -i --rm mcp-assemblyanalyzer-stdio:latest
    ```
