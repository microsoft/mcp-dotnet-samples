...
1. Run the MCP server app in a container.
   ...
   **Parameters**:
   ...
   docker run -i --rm -p 8080:8080 -v "$REPOSITORY_ROOT/openapi-to-sdk/workspace:/app/workspace"  -e HOST_ROOT_PATH="$REPOSITORY_ROOT/openapi-to-sdk" openapi-to-sdk:latest --http -c
   ```

   <!-- ```bash
   # use container image from the container registry
   docker run -i --rm -p 8080:8080 -v "$REPOSITORY_ROOT/openapi-to-sdk/workspace:/app/workspace"  -e HOST_ROOT_PATH="$REPOSITORY_ROOT/openapi-to-sdk" ghcr.io/microsoft/mcp-dotnet-samples/openapi-to-sdk:latest --http -c
   ``` -->
